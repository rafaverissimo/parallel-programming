...
void UpdtBegin(int width, int height,char* ghostbegin, char* gnext, char* gold)
{
    for (int j = 0; j < 2*width; j++)
    {
        if (ghostbegin[j] == inhabitable || gnext[j] == inhabitable)gnext[j] = inhabitable;
        else if ((ghostbegin[j] == habitee || gnext[j] == habitee) &&  gold[j] == habitable )gnext[j] = habitee;
        else if ((ghostbegin[j] == habitable || gnext[j] == habitable) &&  gold[j] == habitee )gnext[j] = habitable;
    }
    
}

void UpdtEnd(int width, int height,char* ghostend, char* gnext, char* gold)
{
    for (int j = 0; j < 2*width; j++)
    {
        if (ghostend[j] == inhabitable || gnext[(height-2)*width+j] == inhabitable) 
            gnext[(height-2)*width+j] = inhabitable;

        else if ((ghostend[j] == habitee || gnext[(height-2)*width+j] == habitee) && gold[(height-2)*width+j] == habitable)
            gnext[(height-2)*width+j] = habitee;
            
        else if ((ghostend[j] == habitable || gnext[(height-2)*width+j] == habitable) && gold[(height-2)*width+j] == habitee) 
            gnext[(height-2)*width+j] = habitable;
